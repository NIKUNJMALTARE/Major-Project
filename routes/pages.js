...
router.get("/user/:username", async (req, res, next) => {
  try {
    let token = req.cookies.AITRConnect;
    alumni.findByToken(token, (err, det) => {
      if (err) return res(err);
      if (det) {
      ...
      if (det) {
        ...
        post.find(
          ...
          (err, doc) => {
            ...
            if (doc)
              return res.render("post", { doc: doc[0], content: doc[0].post });
            else return res.redirect("/notfound");
          }
        );
      } else return res.redirect("/login");
    ...
    if (user) return res.redirect("/dashboard");
      ...
      alumni.findOne({ email: req.body.email }, function (err, user) {
        ...
        user.comparepassword(req.body.password, (err, isMatch) => {
          if (!isMatch)
            return res.render("form", { message: "password doesn't match" });

          user.generateToken((err, user) => {
            if (err) return res.render("form");
            res
              .cookie("AITRConnect", user.token)
...
router.get("/comingsoon", async (req, res, next) => {
  try {
    res.render("error", { title: "Under Developement! Coming Soon 🚧" });
  } catch (error) {
    next(error);
  }
});
...
router.get("/admin-access", async (req, res, next) => {
  try {
    ...
    alumni.findByToken(token, (err, user) => {
      ...
      if (user.__t !== "Admin" && !user.isAdmin) {
        user.isAdmin = true;
        user.save();
